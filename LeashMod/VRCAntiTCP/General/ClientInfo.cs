...
public class ClientInfo
{
    ...
    internal static int NextID = 100;
    ...
    internal bool EncryptionReady => encComplete;

    internal ICryptoTransform Encryptor => encryptor;

    internal ICryptoTransform Decryptor => decryptor;
    ...
    internal ClientDirection Direction => dir;

    internal Socket Socket => sock;

    internal Server Server => server;

    internal int ID => id;

    internal bool Closed => !sock.Connected;

    internal event ConnectionRead OnRead;

    internal event ConnectionClosed OnClose;

    internal event ConnectionReadBytes OnReadBytes;

    internal event ConnectionReadMessage OnReadMessage;

    internal event ConnectionReadPartialMessage OnPartialMessage;

    internal ClientInfo(Socket cl, bool StartNow)
        : this(cl, null, null, ClientDirection.Both, StartNow, EncryptionType.None)
    {
    }

    internal ClientInfo(Socket cl, ConnectionRead read, ConnectionReadBytes readevt, ClientDirection d, bool StartNow)
        : this(cl, read, readevt, d, StartNow, EncryptionType.None)
    {
    }

    internal ClientInfo(Socket cl, ConnectionRead read, ConnectionReadBytes readevt, ClientDirection d, bool StartNow, EncryptionType encryptionType)
    {
        sock = cl;
        buffer = "";
        OnReadBytes = readevt;
    ...
    internal void SendMessage(uint code, byte[] bytes)
    {
        SendMessage(code, bytes, 0, bytes.Length);
    }

    internal void SendMessage(uint code, byte[] bytes, byte paramType)
    {
        SendMessage(code, bytes, paramType, bytes.Length);
    }

    internal void SendMessage(uint code, byte[] bytes, byte paramType, int len)
    {
        if (paramType > 0)
        {
    ...
    internal void Send(byte[] bytes, int len)
    {
        if (encType != 0)
        {
    ...
    internal static void LogBytes(byte[] buf, int len)
    {
        byte[] array = new byte[len];
        Array.Copy(buf, array, len);
        Console.WriteLine(ByteBuilder.FormatParameter(new Parameter(array, 4)));
    }
    ...
    internal static byte[] GetLengthEncodedVector(byte[] from)
    {
        int num = from.Length;
        if (num > 255)
        {
    ...
    internal static int GetInt(byte[] ba, int from, int len)
    {
        int num = 0;
        for (int i = 0; i < len; i++)
        {
    ...
    internal static int[] GetIntArray(byte[] ba)
    {
        return GetIntArray(ba, 0, ba.Length);
    }

    internal static int[] GetIntArray(byte[] ba, int from, int len)
    {
    ...
    internal static uint[] GetUintArray(byte[] ba)
    {
    ...
    internal static byte[] UintToBytes(uint val)
    {
    ...
    internal static byte[] IntArrayToBytes(int[] val)
    {
    ...
    internal static byte[] UintArrayToBytes(uint[] val)
    {
        byte[] array = new byte[val.Length * 4];
    ...
    internal static byte[] StringArrayToBytes(string[] val, Encoding e)
    {
    ...
    internal static string[] GetStringArray(byte[] ba, Encoding e)
    {
