...
namespace VRCAntiTCP.General
{
    ...
    {
        internal static SocksProxy SocksProxy;

        internal static bool UseSocks = false;
        ...
        internal static Socket CreateTCPSocket(string address, int port)
        {
            return CreateTCPSocket(address, port, UseSocks, SocksProxy);
        }

        internal static Socket CreateTCPSocket(string address, int port, bool useSocks, SocksProxy proxy)
        {
            Socket socket;
            ...
            {
                IPAddress address2 = IPAddress.TryParse(address, out var IP) ? IP : Dns.GetHostEntry(address).AddressList[0];
        ...
        internal static Socket ConnectToSocksProxy(IPAddress proxyIP, int proxyPort, string destAddress, int destPort,
